...
CUSTOM_ALIVE = (
    Var.CUSTOM_ALIVE
    if Var.CUSTOM_ALIVE
    else "Hey! Devil is Alive Yet. Ehna Sokha ni Devil Nu marna!"
)
...
async def amireallyalive(alive):
    ...
    with BytesIO() as sticker:
        ...
        await borg.send_message(
            alive.chat_id,
            f"**Welcome To DevilUserBot **\n\n"
            f"`{CUSTOM_ALIVE}`\n\n"
            f"{telemoji} **Telethon version**: `1.17`\n{telemoji} **Python**: `3.8.3`\n"
            f"{telemoji} **DevilUserBot Version**: `{telever}`\n"
            f"{telemoji} **More Info**: @Deviluserbot\n"
            f"{telemoji} **Sudo** : `{sudo}`\n"
            f"{telemoji} **DevilUserBot Uptime**: `{uptime}`\n"
            f"{telemoji} **Database Status**: `Cato On Flower👌!`\n"
            f"{telemoji} **My pro owner** : [{DEFAULTUSER}](tg://user?id={myid})\n\n"
